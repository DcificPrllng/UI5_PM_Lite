sap.ui.define([
	...
	"sap/m/BusyDialog"
], function (BaseController, JSONModel, formatter, MessageBox, Filter, MessageToast, Button, TreeTable, BusyDialog) {
	"use strict";
	return BaseController.extend("pd.pm.lite.controller.Home", {
		...
		onInit: function () {
			...
			//Busy dialog while navigating
			this.getOwnerComponent().BusyDialogGlobal = new BusyDialog("GlobalBusyDialog",{title:"Loading. . . "});
		...
		triggerPrint: function () {
			...
			oModel.callFunction("/PrintWorkOrders", {
				...
				success: function (oData, response) {
					oView.setBusy(false);
					MessageToast.show("Orders successfully sent to the printer");
				},
		...
		gotoIW31: function (evt) {
			...
			var selectedContext = hierarchyTable.getContextByIndex(hierarchyTable.getSelectedIndex());

			//Check if nothing was selected
			if (!selectedContext) {
				MessageBox.error(
					"Select a Functional Location or an Equipment"
				);
				return;
			}
		...
		createHierarchyPopup: function (plant) {
			...
			var oHierarchyTable = new TreeTable({
			...
			var oHierarchyPopup = new sap.m.Dialog({
				title: "Select a Functional Location/Equipment",
				width: "768px",
				contentHeight: "95%",
				...
				buttons: [
					new Button({
						text: "Cancel",
						press: [controller.closePopUp, controller]
					}),
					new Button({
						text: "Create Order",
						type: "Emphasized",
